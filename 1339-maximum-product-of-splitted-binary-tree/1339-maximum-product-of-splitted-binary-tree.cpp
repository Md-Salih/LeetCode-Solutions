...
public:
    int maxProduct(TreeNode* root) {
        ...
        function<ll(TreeNode*)> calculateTotalSum = [&](TreeNode* node) -> ll {
            ...
            // Recursive case: current node value + sum of left subtree + sum of
            // right subtree
            return node->val + calculateTotalSum(node->left) +
        ...
        function<ll(TreeNode*)> findMaxProduct = [&](TreeNode* node) -> ll {
            ...
            ll currentSubtreeSum = node->val + findMaxProduct(node->left) +
